...
const { User, Institution, Teacher, Student } = require('../models');
...
const register = async (req, res, next) => {
  try {
    const { name, email, password, institutionName, role } = req.body;
    if (!name || !email || !password) return res.status(400).json({ error: 'name, email, password required' });

    const existing = await User.findOne({ where: { email } });
    if (existing) return res.status(409).json({ error: 'Email already registered' });

    // Security: Only 'admin' can self-register for the system setup
    if (role !== 'admin') {
      return res.status(403).json({ error: 'Only Administrators can register. Teachers must be added by an admin.' });
    }

    // Get or create the single institution for this system
    let institution = await Institution.findOne();
    if (!institution) {
      institution = await Institution.create({ 
        name: institutionName || 'Default University', 
        code: 'PRIMARY' 
      });
    }
